        ...
        async fn handle_request_impl(session: &mut crate::session::Session, mut msg: Message) -> Result<(), crate::session::SessionError> {
            ...
            ::paste::paste! {
                match msg.get_message_id() {
                    $(
                        ::wicked_waifus_protocol::[<$name Request>]::MESSAGE_ID => {
                            let request = ::wicked_waifus_protocol::[<$name Request>]::decode(&*msg.remove_payload())?;
                            let mut response = ::wicked_waifus_protocol::[<$name Response>]::default();
